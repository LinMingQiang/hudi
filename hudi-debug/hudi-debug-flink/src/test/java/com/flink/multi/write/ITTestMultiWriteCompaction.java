...
/**
 * 问题1：coor的时候，如果有一个任务有数据，两个job都会进行调度吧，这时候生成两个 compaction.request?
 *  -- 不会生成两个request，不知道为什么第二个虽然进去调度了，但是没有生成plan，要看remotefilesysview怎么回事。
 * 问题2：CompactionPlanOperator.notifyCheckpointComplete.scheduleCompaction 会对同一个request做inflight导致冲突
 *
 */
public class ITTestMultiWriteCompaction {
    public static StreamTableEnvironment tableEnv;

    public static String PATH = "file:///Users/hunter/workspace/hudipr/multiwriter-0.12.0-5/hudi-debug/target/ITTestMultiWriteCompaction";
